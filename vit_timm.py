...
default_cfgs = {
    ...
    'vit_small_patch3_42': _cfg(
        url='', input_size=(3, 42, 42), crop_pct=1.0),
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, is_last=False):
        super().__init__()
        ...
        self.norm2 = norm_layer(dim) if not self.is_last else nn.Identity()
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

    def forward(self, x, get_attn=False):
        if not get_attn:
            x = x + self.drop_path(self.attn(self.norm1(x))) if not self.is_last else self.attn(self.norm1(x))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x
        else:
            y, attn = self.attn(self.norm1(x), get_attn=True)
            x = x + self.drop_path(y) if not self.is_last else y
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x, attn
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', use_nosn=False):
        """
        Args:
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                is_last= i == depth-1 if use_nosn else False)
...
@register_model
def vit_small_patch3_42(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    NOTE I've replaced my previous 'small' model definition and weights with the small variant from the DeiT paper
    """
    model_kwargs = dict(patch_size=3, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_vision_transformer('vit_small_patch3_42', pretrained=pretrained, **model_kwargs)
    return model
